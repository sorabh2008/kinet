...
### Architecture
How this fits into the existing Python package/module structure.
...
### Dependencies
Any new packages needed — check `requirements.txt`/`pyproject.toml` first to avoid duplicating an existing dependency.

### Tests
What tests are required. Follow the project's testing conventions (pytest/unittest).
